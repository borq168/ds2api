...
func TestHandleResponsesStreamWithRetryDoesNotRetryAfterRequestContextCanceled(t *testing.T) {
	...
	resp := &http.Response{
		...
		Body:       io.NopCloser(strings.NewReader(`data: {"response_message_id":2,"p":"response/thinking_content","v":"plan"}` + "\n" + "data: [DONE]\n")),
	}

	h.handleResponsesStreamWithRetry(
		rec,
		req,
	...
	if strings.Contains(rec.Body.String(), "response.failed") {
		t.Fatalf("did not expect retry failure event after cancellation, body=%s", rec.Body.String())
	}
}
